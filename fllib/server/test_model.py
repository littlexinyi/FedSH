...
def test(config, device, round, network, img_dataloader, txt_dataloader, save_path, save_file):
    ...
    if(config.server.aggregation_detail.PRL):
        image_feature_non_local = torch.cat(image_feature_non_local, 0)
    else:
        image_feature_non_local = image_feature_local
    ...
    for times, [label, caption_code, caption_length] in enumerate(txt_dataloader):
        label = label.to(device)
        caption_code = caption_code.to(device).long()
    ...
    if(config.server.aggregation_detail.PRL):
        ...
        text_feature_non_local = text_feature_local
