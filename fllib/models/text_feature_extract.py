...
class TextExtract(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.embedding_local = nn.Embedding(config.dataset.vocab_size, 512, padding_idx=0)
        self.embedding_global = nn.Embedding(config.dataset.vocab_size, 512, padding_idx=0)
    ...
    def calculate_different_length_lstm(self, text_embedding, text_length, lstm):
        ...
        sort_text_length = text_length[sort_index].cpu()
